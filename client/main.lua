...
local KEY_LAST_INDEXES = "clothing-last-indexes"
local KEY_LAST_TEXTURE_INDEXES = "clothing-last-texture-indexes"
...
local function OpenMenu()
    ...
    CamHandle = CreateCamWithParams('DEFAULT_SCRIPTED_CAMERA', camCoords.x, camCoords.y, camCoords.z, 0.0, 0.0,
        pedHeading, CurrentFov, false, 0)
...
RegisterNuiCallback('changeClothesState', function(data, cb)
    ...
    local lastIndexesState = LocalPlayer.state[KEY_LAST_INDEXES]
    local lastTextureIndexesState = LocalPlayer.state[KEY_LAST_TEXTURE_INDEXES]
    if lastIndexesState then
        LastIndexes = lastIndexesState
    end
    if lastTextureIndexesState then
        LastTextureIndexes = lastTextureIndexesState
    end
    if LastIndexes[data.clothing] then
        ...
        TaskPlayAnim(PlayerPedId(), animation.dict, animation.name, 3.0, 3.0, animation.duration, animation.move, 0,
            false, false, false)
        AnimPlaying = true
        local pause = animation.duration - 500
        if pause < 500 then pause = 500 end
        Wait(pause)
        AnimPlaying = false
        if ClothingTable[data.clothing].type == 'component' then
            SetPedComponentVariation(PlayerPedId(), ClothingTable[data.clothing].id, LastIndexes[data.clothing],
                LastTextureIndexes[data.clothing])
        else
            SetPedPropIndex(PlayerPedId(), ClothingTable[data.clothing].id, LastIndexes[data.clothing],
                LastTextureIndexes[data.clothing])
        end
        ...
        if ClothingTable[data.clothing].type == 'component' then
            ...
            SetPedComponentVariation(PlayerPedId(), ClothingTable[data.clothing].id,
                Config.Clothing[data.clothing].default, 0)
        else
    ...
    LocalPlayer.state[KEY_LAST_INDEXES] = LastIndexes;
    LocalPlayer.state[KEY_LAST_TEXTURE_INDEXES] = LastTextureIndexes;
end)
